...
class AbstractPlayer(ABC):
    ...
    def __str__(self) -> str:
        player_name = getattr(self, 'name', self.__class__.__name__)
        return f"{player_name}(score={self.get_score()})"
...
class UserPlayer(AbstractPlayer):
    ...
    def make_move(self, game_board: GameBoard) -> Move:
        while True:
            try:
                x: int = int(
                    input(f"Enter Row Position 0 - 4: ")
                )
                y: int = int(
                    input(f"Enter Column Position 0 - 4: ")
                )
